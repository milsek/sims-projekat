<template>
  <div
    class="block md:flex mx-auto rounded-xl border border-dashed border-gray-300 p-6"
  >
    <div class="flex-shrink-0">
      <img
        :src="data.editionImageLarge"
        alt=""
        class="text-center sm:text-left rounded-md
      w-36 lg:w-40 h-56 lg:h-60 object-fill mx-auto"
        draggable="false"
      />
    </div>
    <div class="md:pl-4 text-center md:text-left" v-if="data.bookState">
      <div
        class="text-xl md:text-base lg:text-xl font-bold text-blue-900 opacity-90
      ...
      <div class="mt-2 text-sm font-medium tracking-wide text-gray-500 ">
        ...
        <div class="line-clamp-2">
          Isle: {{ data.lineIsleName }} - Row: {{ data.lineNumber }}
        </div>
        <div class="line-clamp-2">
          Condition:
          <span :class="[colorCondition]" class="font-bold">{{
            data.condition
          }}</span>
        </div>
      ...
      <div v-if="data.bookState === 'IN_STOCK'" class="mt-5">
        <button
          ...
          class="h-9 mt-4 px-8 md:px-6 bg-blue-700 hover:bg-blue-900
      ...
      <div v-if="data.bookState === 'TAKEN'" class="mt-5">
        <button
          @click="markBookReturned"
          class="h-9 mt-4 px-8 md:px-6 bg-gray-700 hover:bg-gray-800
         opacity-90 text-white text-center text-lg shadow-md focus:outline-none rounded-lg"
        >
          mark returned
        </button>
  ...
  methods: {
    ...
    markBookReturned() {
      ...
      axios
        ...
        .then(x => {
          that.data.bookState = "IN_STOCK";
          that.justReturned = true;
        })
        .catch(error => {
